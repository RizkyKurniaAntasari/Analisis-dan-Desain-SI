        ...
        <div class="p-4 border border-green-200 rounded-b-lg bg-green-50">
            <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" class="w-full p-2 border border-gray-300 rounded-md bg-green-100" value="{{ old('title', $article->title) }}" required>
                    @error('title')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="author" id="author" class="w-full p-2 border border-gray-300 rounded-md bg-green-100" value="{{ old('author', $article->author) }}" required>
                    @error('author')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="category" id="category" class="w-full p-2 border border-gray-300 rounded-md bg-green-100" required>
                        ...
                        <option value="Pertanian" {{ old('category', $article->category) == 'Pertanian' ? 'selected' : '' }}>Pertanian</option>
                        <option value="Perkebunan" {{ old('category', $article->category) == 'Perkebunan' ? 'selected' : '' }}>Perkebunan</option>
                    </select>
                    @error('category')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="content" rows="8" class="w-full p-2 border border-gray-300 rounded-md bg-green-100" required>{{ old('content', $article->content) }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-green-700 text-white rounded-md">Simpan</button>
                </div>
            </form>
        </div>
    </div>
